...
class Controller extends BaseController
{
    ...
    public function callApi($url, $post_fields = [], $payment_method = null, $method = null)
    {
        $ch = curl_init();
        ...
        if ($payment_method == 'razorpay') {
            ...
            if (env('RAZORPAY_MODE', 'test') == 'live') {

                $key = env('RAZORPAY_LIVE_KEY');
                $secret = env('RAZORPAY_LIVE_SECRET');

            } else {

                $key = env('RAZORPAY_TEST_KEY');
                $secret = env('RAZORPAY_TEST_SECRET');
            }
            info("-------all keys-----",[
                'mode' => env('RAZORPAY_MODE'),
                'key' => env('RAZORPAY_TEST_KEY'),
                'secret_exists' => !empty(env('RAZORPAY_TEST_SECRET')),
                'url' => env('RAZORPAY_DEFAULT_URL'),
            ]);
    ...
    public function calculateTaxes($price)
    {
        ...
        $data['total_tax'] = ($price - (round(($price / $tax_rate), 2)));
        $data['sgst'] = $data['cgst'] = ($data['total_tax'] / 2);
        return $data;
    }
}