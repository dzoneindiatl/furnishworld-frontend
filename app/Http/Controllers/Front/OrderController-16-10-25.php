...
class OrderController extends Controller
{
    ...
    public function orderDetails(Request $request, $orderId)
    {
        ...
        $html = view('front.modules.dashboard.order-details', compact('orderDetails', 'orderItems', 'paymentMode', 'allStatuses'))->render();
    ...
    public function submitRefundRequest(Request $request)
    {
        ...
        $orderStatus->user_id           = $user_id;
        $orderStatus->order_id          = $order->id;
        $orderStatus->order_status_id   = $status['id'] ?? 8;
        $orderStatus->save();

        $orderitem->order_status_id   = $status['id'] ?? 8;
        $orderitem->save();
